...
- [ ] Paso 2 del onboarding: pantalla real de "tipo de horario" (reemplaza el placeholder en app/onboarding/horario/page.tsx). Define tipo_horario ('fijo' | 'variable') y, según el tipo, deriva a la configuración correspondiente (iCal si variable / bloques por día si fijo). Reusar el patrón de router.refresh() + guarda server tras guardar.
